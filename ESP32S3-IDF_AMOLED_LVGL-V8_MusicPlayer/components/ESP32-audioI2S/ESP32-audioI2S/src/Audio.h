...
public:
  ...
  private:
    ...
    size_t          resampleTo44kStereo(const int16_t* input, size_t inputFrames);
    void            playChunk();
...
#pragma GCC diagnostic pop
    ...
    const size_t    m_samplesBuff44KSize = m_outbuffSize * 8; // 131072KB  SRmin: 6KHz -> SRmax: 44.1K

    static const uint8_t m_tsPacketSize  = 188;
    static const uint8_t m_tsHeaderSize  = 4;

    ps_ptr<int16_t>  m_outBuff;        // Interleaved L/R
    ps_ptr<int16_t>  m_samplesBuff44K; // Interleaved L/R
    ...
    uint32_t        m_sampleRate=48000;             // Actually 44.1KHz
    ...
    int16_t         m_inputHistory[6] = {0};        // used in resampleTo44kStereo()
    ...
    float           m_resampleRatio = 1.0f;         // resample ratio for e.g. 48kHz to 44.1kHz
    float           m_resampleCursor = 0.0f;        // next frac in resampleTo44kStereo
